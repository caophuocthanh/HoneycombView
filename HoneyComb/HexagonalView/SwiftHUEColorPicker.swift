...
//  Edit by Cao Phuoc Thanh on 25/01/2018
...
open class SwiftHUEColorPicker: UIView {
    ...
    open var labelBackgroundColor: UIColor = UIColor.red
    ...
    func generateHUEImage(_ size: CGSize) -> UIImage {
        ...
        let rect = CGRect(x: 0, y: (size.height/2 - size.height/8), width: size.width, height: size.height/4)
    ...
    override open func draw(_ rect: CGRect) {
        ...
        let radius = ((direction == .horizontal ? self.frame.size.height : self.frame.size.width)) - 6
        ...
        let circleRect = (direction == .horizontal ? CGRect(x: circleX, y: 3, width: radius, height: radius) : CGRect(x: 1, y: circleY, width: radius, height: radius))
    ...
    open override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let point = touch?.location(in: self) {
            handleTouch(point)
            self.delegate?.valuePickedEndDrag(color, type: type)
        }
    }
    ...
    func handleTouch(_ touchPoint: CGPoint) {
        ...
        self.labelBackgroundColor = color
        
        if delegate != nil {
            self.delegate?.valuePicked(color, type: type)
        }
